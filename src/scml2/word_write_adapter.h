...
class word_write_adapter : public word_adapter_base<M>
{
public:
...
public:
  word_write_adapter(C& c, CallbackType cb, const M& mem, bool check_word_access = true);
...
{
  return word_write_adapter<M, C>(c, cb, mem, check_word_access);
}

template <typename M, typename C>
inline
word_write_adapter<M, C>::
word_write_adapter(C& c, CallbackType cb, const M& mem, bool check_word_access) :
