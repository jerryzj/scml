...
class word_read_adapter : public word_adapter_base<M>
{
public:
...
public:
  word_read_adapter(C& c, CallbackType cb, const M& mem, bool check_word_access = true);
...
{
  return word_read_adapter<M, C>(c, cb, mem, check_word_access);
}

template <typename M, typename C>
inline
word_read_adapter<M, C>::
word_read_adapter(C& c, CallbackType cb, const M& mem, bool check_word_access) :
