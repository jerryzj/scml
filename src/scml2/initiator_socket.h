...
initiator_socket() :
  ...
  m_mm (tlm2_gp_mm_dispatcher::create_mm())
{
  this->register_invalidate_direct_mem_ptr(this, &initiator_socket::invalidate_direct_mem_ptr);
  this->register_nb_transport_bw(this, &initiator_socket::nb_transport_bw);
  this->register_bw_direct_mem_if(&mDmiHandler);
}
            ...
            bool readNotWrite)
{
  ...
  tlm::tlm_generic_payload* trans = m_mm->claim();
  trans->acquire();
  ...
  bool response = trans->is_response_ok();
  trans->release();
  return response;
                 ...
                 bool readNotWrite)
{
  ...
  unsigned int ret_val =  (*this)->transport_dbg(*trans);
  trans->release();
  return ret_val;
}
