...
checkRange(unsigned long long base, unsigned long long size) const
{
  ...
  if (base % get_width()) {
    SCML2_MODEL_INTERNAL(LEVEL1) << " to-be-mapped range with base=" << base << " and size=" << size << " is not aligned on router width=" << get_width() << std::endl;
    return false;
  }

  // Mapped size must be a multiple of width
  if (size % get_width()) {
    SCML2_MODEL_INTERNAL(LEVEL1) << " to-be-mapped range with base=" << base << " and size=" << size << " is not a multiple of router width=" << get_width() << std::endl;
    return false;
  }

  // Mapped size must fit into size of router object
  if ((base + size) > index_to_address(get_size(), get_width())) {
    SCML2_MODEL_INTERNAL(LEVEL1) << " to-be-mapped range with base=" << base << " and size=" << size << " does not fit in size of router=" << index_to_address(get_size(), get_width()) << std::endl;
    return false;
  }
             ...
             unsigned long long size) const
{
  ...
  for (; it != end; ++it) {
    ...
    if ((iterLow <= high) && (iterHigh >= low)) {
      SCML2_MODEL_INTERNAL(LEVEL1) << " to-be-mapped range with base=" << base << " and size=" << size << " overlaps with previously mapped range with base=" << range->get_base() << " and size=" << range->get_size() << std::endl;
      return true;
    }
  }
    ...
    unsigned long long offset)
{
  ...
  mapped_range* read_range = new mapped_range(*this, base, size, dest, offset);
  mMappedReadRanges.insert(mMappedReadRanges.begin(), read_range);
  dest.register_bw_direct_mem_if(read_range);
  mapped_range* write_range = new mapped_range(*this, base, size, dest, offset);
  mMappedWriteRanges.insert(mMappedWriteRanges.begin(), write_range);
  dest.register_bw_direct_mem_if(write_range);
...
unmap_all()
{
  ...
  MappedRangesConstIter end = mMappedReadRanges.end();
  for (; it != end; ++it) {
    (*it)->unregister_bw_direct_mem_if(*it);
    delete *it;
  }
  mMappedReadRanges.clear();
...
b_transport(tlm::tlm_generic_payload& trans, sc_core::sc_time& t)
{
  ...
  if (range) {
    ...
    if (highIndex >= get_size()) {
      SCML2_ERROR(ACCESS_UNMAPPED_ADDRESS) << "Access to address 0x" << std::hex << address << "-0x" << endAddress << " is out of bounds." << std::endl;
