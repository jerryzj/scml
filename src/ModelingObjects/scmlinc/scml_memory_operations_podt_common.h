...
template< typename DT > 
class SNPS_VP_API scml_memory_operations_podt_common :
...
public:
  ...
  // access methods
  inline value_type operator[] ( size_type ) const ;

  inline value_type get( size_type a = 0 ) const ;
  inline value_type get( size_type, unsigned int acSiz, unsigned int offSet) const ;
  inline void put( value_type, size_type a = 0 );
  inline void put( value_type, size_type, unsigned int acSiz, unsigned int offSet);
  ...
  // bit access
  inline void put_bits( value_type val, size_type index, unsigned int offset, unsigned int nBits );
  inline value_type get_bits( size_type index, unsigned int offset, unsigned int nBits ) const;
  ...
  // decrement & increment
  inline this_reference_type operator -- (); // prefix
  inline value_type operator -- ( int);      // postfix
  inline this_reference_type operator ++ (); // prefix
  inline value_type operator ++ ( int);      // postfix
  ...
  inline uint64 indexToByteAddress( size_type i,
				    unsigned int accessSize,
				    unsigned int offSet) const;
  ...
  inline void registerTransportCB(MOD * m,
				  RESP (MOD::*f) ( const REQ & ),
				  bool isBlocking = true,
				  user_id_data_type id = 0,
				  const std::string& cbName = "");
  template< typename MOD >
  inline void registerReadCB( MOD * m, 
			      ...
			      const std::string& cbName = "");
  template< typename MOD >
  inline void registerWriteCB( MOD * m, 
...
protected:
  ...
  template< typename IF >
  typename IF::data_type readBitfield(unsigned int dataSize,
                                      unsigned int offSet);
  template< typename IF >
