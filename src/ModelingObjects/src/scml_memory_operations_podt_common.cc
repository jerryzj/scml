...
namespace scml_memory_operations_impl_helpers {
  ...
  class Normal
  {
    public:
      ...
      typedef unsigned long long size_type;
    public:
      Normal(vector_value_type& vector, size_type index) : mVector(vector), mOffset((unsigned int)index) {}
      ~Normal() {}
      
      value_type getData(size_type index = 0) const { return mVector[(unsigned int)(mOffset+index)]; }
      void setData(value_type value, size_type index = 0) { mVector[(unsigned int)(mOffset+index)] = value; }
  ...
  class ImplSelector
  {
    public:
      ...
      { return Selected(reinterpret_cast<unsigned char*>(&v[(unsigned int)index]), 0); } 
      typedef FastSwap<DT> SelectedSwap;
      static SelectedSwap createImplSwap(vector_value_type& v, size_type index)
      { return SelectedSwap(reinterpret_cast<unsigned char*>(&v[(unsigned int)index]), 0); } 
  };
  ...
  mMapableIf(new scml_mapable_if_to_T<this_type>(*this))
{
  ...
  mSharedData->spm = new vector_value_type((size_t)a);
                                                  ...
                                                  bool isDebug)
{
  ...
  req.setAddress((typename IF::address_type)address);
...
transportProp( const typename IF::request_type & req) {
  ...
  req.m_address -= (typename IF::address_type)indexToAddress( p->memOffset);
...
template< typename DT >
template< typename IF >
typename IF::data_type
scml_memory_operations_podt_common< DT >::
readBitfield(unsigned int dataSize,
             unsigned int offSet)
{
  value_type readData(getDebug());
  const value_type mask(bitfieldContainer->read(readData, dataSize, offSet));
  const value_type result((getDebug() & ~mask) | (readData & mask));

  return convert<value_type, typename IF::data_type>(scml_memory_utils::extractBits(result, offSet, dataSize));
}
...
transportImp( const typename IF::request_type & req) {
  ...
  if( pvWrite == reqType) {
    ...
    if( pvDebug != reqMode || trigger_behaviour_on_debug_read()) {
      ...
      if( haveBitfieldsWithCallbacks) {
        resp.setReadData( readBitfield<IF>(dataSize, offSet));
        resp.setLatency( this->get_default_response_latency());
        resp.setResponse( this->get_default_response_status());
        return resp;
      }
	  ...
	  readResp = mTransportCb->transport(req);
        ...
        if( store_after_read_call() && !user.is_debug_access()) {
          for( size_t i( 0); i < burstCount; ++i) { 
            put( convert<typename IF::data_type, value_type>(readResp.getReadData( (unsigned int) i )), index + i, dataSize, offSet);
          }
        }
        return readResp;
      }
...
getAliasPointers(memory_pointer_container& v) const
{
  ...
  return (unsigned int) aliases.size();
}
...
template class SNPS_VP_API scml_memory_operations_podt_common< sc_dt::sc_biguint<128> >;
template class SNPS_VP_API scml_memory_operations_podt_common< sc_dt::sc_biguint<256> >;
template class SNPS_VP_API scml_memory_operations_podt_common< sc_dt::sc_biguint<512> >;
