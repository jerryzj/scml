...
collect_mapped_read_ranges(std::vector<scml_mapped_range*>& v) const
{
  ...
  return (unsigned int) mMappedReadRanges.size();
}
...
collect_mapped_write_ranges(std::vector<scml_mapped_range*>& v) const
{
  ...
  std::copy(mMappedWriteRanges.begin(),
            ...
            std::back_insert_iterator<std::vector<scml_mapped_range*> >(v));
  return (unsigned int) mMappedWriteRanges.size();
}
...
template class SNPS_VP_API scml_router_operations_podt_common< sc_dt::sc_biguint<128> >;
template class SNPS_VP_API scml_router_operations_podt_common< sc_dt::sc_biguint<256> >;
template class SNPS_VP_API scml_router_operations_podt_common< sc_dt::sc_biguint<512> >;
