...
{
  if (mModule) {
    mValue = (int) scml_property_registry::inst().getIntProperty(
...
{
  if (mModule) {
    mValue = (unsigned int) scml_property_registry::inst().getUIntProperty(
    	scml_property_registry::MODULE, mModule->name(), mName);
  }
}
...
#define SLS_STRING_OPERATOR_IMPLEMENTATIONS(otype)\
...
SLS_STRING_OPERATOR_IMPLEMENTATIONS(==);
SLS_STRING_OPERATOR_IMPLEMENTATIONS(!=);
SLS_STRING_OPERATOR_IMPLEMENTATIONS(<);
SLS_STRING_OPERATOR_IMPLEMENTATIONS(<=);
SLS_STRING_OPERATOR_IMPLEMENTATIONS(>);
...
#undef SLS_STRING_OPERATOR_IMPLEMENTATIONS
